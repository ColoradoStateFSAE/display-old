...
class Dashboard:
    def __init__(self, x, y, screen):
        ...
        self.manifold = dashboard_elements.Guage("MAP", "right", right, row_1, screen, units="kPa")
        self.clt = dashboard_elements.Guage("ENGINE", "left", left, row_2, screen, units="°F")
        self.tps = dashboard_elements.Guage("TPS", "right", right, row_2, screen, units="%")
        self.afr = dashboard_elements.Guage("AFR", "left", left, row_3, screen, decimals=1)
        self.adv = dashboard_elements.Guage("ADVANCE", "right", right, row_3, screen, units="°")
        self.sync = dashboard_elements.Guage("SYNC-LOSS", "left", left, row_4, screen)
        self.sat = dashboard_elements.Guage("SAT", "right", right, row_4, screen)

        self.intro = dashboard_elements.Fade(14, x, y, screen)

        self.wifi = dashboard_elements.WiFi(x//2, y-30, screen)
        
        self.wifi_active = False
        threading.Thread(target=self.__check_wifi, daemon=True).start()
        
    def update(self, can_data, gps_data):
        ...
        self.sat.update(gps_data.sats.value)
