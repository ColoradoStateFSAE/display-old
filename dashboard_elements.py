...
class Satellites():
    def __init__(self, x, y, screen):
        self.icon = pygame.image.load(cwd + "/image/satellite.png")
        self.font = pygame.font.Font(style.bold_font, 20)
    ...
    def show(self):
        icon_rect = self.icon.get_rect(bottomleft=(self.x, self.y))
        self.screen.blit(self.icon, icon_rect)
        
        text = self.font.render(self.value, 1, black)
        text_rect = text.get_rect(bottomleft=(self.x+10, self.y-16))
