...
if __name__ == '__main__':
    ...
    can_filename = log_path + "/{}can.csv".format(filename)
    gps_filename = log_path + "/{}gps.nmea".format(filename)
    motion_filename = log_path + "/{}motion.csv".format(filename)
    ...
    while True:
        if(time.time() - can_data.last_update.value >= 3 or not can_data.has_data.value):
            ...
            dashboard.update(can_data, gps_data, filename)
            dashboard.show()
